...
class GlassdoorScraper(Scraper):
    ...
    def scrape(self, scraper_input: ScraperInput) -> JobResponse:
        ...
        if location_type is None:
            ...
            f = open("localisation_inconnues_glassdoor.txt","a")
            f.write(scraper_input.location+"\n")
            f.close()
        ...
        while len(job_list) < scraper_input.results_wanted:
            ...
            try:
                nbr, jobs, cursor = self._fetch_jobs_page(
                    scraper_input, location_id, location_type, page, cursor
                )
                
                if nbr>0:
                    logger.info(f"Glassdoor, {nbr} jobs, page {page}, save {len(jobs)}")
                job_list += jobs
                if nbr<100 or page>20 :
                    break
                page += 1
    ...
    def _fetch_jobs_page(
        ...
        return len(jobs_data), job_list, self.get_cursor_for_page(
    ...
    def _process_job(self, job_data):
        ...
        if(self.historique.ajoutId(str(job_id))):
            return
    ...
    def _get_location(self, location: str, is_remote: bool) -> (int, str):
        ...
        if not items:
            #logger.error(f"Location '{location}' not found on Glassdoor")
            return None, None
    ...
    def _add_payload(
        ...
        if self.scraper_input.start_hours:
            fromage = max(self.scraper_input.start_hours // 24, 1)
        filter_params = []
